...
class ProduitController extends AbstractController
{
    ...
    public function index(
        ...
        $sort = in_array($sort, $validSorts, true) ? $sort : 'nomProduit';
        ...
        return $this->render('produit/index.html.twig', [
            'produits' => $produits,
            'categories' => $categorieRepository->findAllOrdered(50),
    ...
    public function show(
        ...
        return $this->render('produit/show.html.twig', [
            ...
            'aiInsight' => $productCatalogAiService->buildInsight($produit),
        ]);
    }
    ...
    private function buildAiCategorySuggestion(
        ...
        $productName = trim((string) $request->request->get('nomProduit', ''));
        $address = trim((string) $request->request->get('adresse', ''));

        if ($productName === '') {
            return null;
        }

        return $productCatalogAiService->suggestCategory($productName, $address, $categories);
    }

    private function resolveSelectedCategoryId(Request $request, ?array $aiCategorySuggestion): ?int
    {
        $postedCategoryId = (string) $request->request->get('categorie', '');

        if (ctype_digit($postedCategoryId)) {
            return (int) $postedCategoryId;
        }

        $suggestedCategory = $aiCategorySuggestion['category'] ?? null;

        if ($suggestedCategory instanceof Categorie) {
            return $suggestedCategory->getId();
        }
    ...
    private function applyProductRequestData(
        Produit $produit,
        Request $request,
        CategorieRepository $categorieRepository
    ): array {
